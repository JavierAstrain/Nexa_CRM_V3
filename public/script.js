...
function renderContact(contact) {
  ...
  root.querySelector('.summarize').addEventListener('click', async () => {
    ...
    try {
      ...
      summaryEl.textContent = 'Error al generar resumen.';
    }
  });
...
function renderOpp(opp) {
  ...
  descEl.value = opp.description || '';
  const probEl = root.querySelector('.probability');
  probEl.value = String(opp.probability || 0);
  const stageEl = root.querySelector('.stage');
  if (stageEl) stageEl.value = opp.stage || 'Prospecto';
  ...
  root.querySelector('.predict').addEventListener('click', async () => {
    try {
      ...
      alert('Error al predecir probabilidad');
    }
  });

  // Simple blur save for description/probability
...
document.getElementById('btn-new-opp').addEventListener('click', async () => {
  const title = prompt('Título de oportunidad');
  if (!title) return;
...
document.getElementById('ai-advise')?.addEventListener('click', async () => {
  ...
  try {
    ...
    out.textContent = 'Error al analizar.';
  }
});
